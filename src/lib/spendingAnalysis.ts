import { MonthlyStats, YearlyStats } from "@/types/api";
...
const DEFAULT_OPTIONS: SpendingAnalysisOptions = {
  singleTransactionThreshold:
    appConfig.spendingAnalysis.singleTransactionThreshold.default,
  monthlyIncreaseThreshold:
    appConfig.spendingAnalysis.monthlyIncreaseThreshold.default,
  yearlyIncreaseThreshold:
    appConfig.spendingAnalysis.yearlyIncreaseThreshold.default,
  budgetUsageThreshold: appConfig.spendingAnalysis.budgetUsageThreshold.default,
};

export const SPENDING_ANALYSIS_CONFIG = {
  singleTransactionThreshold:
    appConfig.spendingAnalysis.singleTransactionThreshold,
...
export class SpendingAnalyzer {
  ...
  checkYearlyIncrease(
    ...
    const increase =
      ((currentYear.totalExpense - previousYear.totalExpense) /
        previousYear.totalExpense) *
      100;
  ...
  async checkBudgetUsage(
    year: number,
    month: number,
    categoryStats: Record<string, { amount: number }>,
  ...
  async analyzeSpending(
    ...
    // 检查预算使用情况
    const budgetAlerts = await this.checkBudgetUsage(
      year,
      month,
      currentMonth.categoryStats,
    );
