...
import { filterExpensesByThreshold, getExpenseSummary } from '../utils/expenseUtils';
...
const ExpenseBreakdown: React.FC<ExpenseBreakdownProps> = ({ expenses }) => {
  ...
  const aboveThresholdExpenses = filterExpensesByThreshold(expenses, threshold, true);
  const belowThresholdExpenses = filterExpensesByThreshold(expenses, threshold, false);

  const aboveSummary = getExpenseSummary(aboveThresholdExpenses);
  const belowSummary = getExpenseSummary(belowThresholdExpenses);

  const renderCategoryList = (categories: Record<string, number>, total: number) => (
      ...
      {Object.entries(categories)
        ...
        .map(([category, amount]) => (
          <div key={category} className="flex justify-between">
            <span>{category}</span>
            <span className="font-medium">¥{amount.toFixed(2)} ({(amount / total * 100).toFixed(1)}%)</span>
          </div>
        ))}
    </div>
  );
  ...
  return (
    <div className="flex flex-col md:flex-row gap-4">
      <div className="p-4 bg-blue-50 rounded-lg flex-1">
        <h3 className="text-lg font-medium text-blue-800 mb-2">大额支出 (≥ ¥{threshold})</h3>
      ...
      <div className="p-4 bg-green-50 rounded-lg flex-1">
        <h3 className="text-lg font-medium text-green-800 mb-2">小额支出 (&lt; ¥{threshold})</h3>
        <div className="flex justify-between mb-4">
