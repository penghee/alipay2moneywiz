...
import { ArrowLeft, PlusCircle, DollarSign, Calendar as CalendarIcon, Tag, User, CreditCard, ArrowDownUp, FileText, MessageSquare, Hash, ChevronDown } from 'lucide-react';

// Import the data files
type CategoryMap = Record<string, string[]>;
type BillOwner = { id: string; name: string };
type BillOwnersData = { defaultOwner: string; owners: BillOwner[] };

const categoryMap: CategoryMap = require('@/config/category_map.json');
const billOwnersData: BillOwnersData = require('@/config/bill_owners.json');
    ...
    try {
      ...
      const transaction = {
        ...
        amount: formData.type === 'expense' 
          ? `-${Math.abs(Number(formData.amount)).toFixed(2)}`
          : Math.abs(Number(formData.amount)).toFixed(2),
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                <label className="block text-sm font-medium text-gray-700">
                  ...
                  <span className="text-xs text-gray-500 font-normal">多个标签用逗号分隔</span>
                </label>
                <input
                  type="text"
                  name="tags"
